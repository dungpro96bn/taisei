<div id="top-interview">
    <div class="inner">
        <div class="header-entry" data-aos="fade-up">
            <h2 class="heading-block heading-center">
                <span class="ttl-ja">社員インタビュー</span>
            </h2>
            <p class="subTitle">皆さんの先輩の働き方や志望動機などを<br class="sp-br"/>ご紹介します</p>
        </div>
        <div class="interview-list">
            <div class="interview-item">
                <a href="{{route('interview')}}/#interview-01">
                    <picture>
                        <source media="(max-width: 767px)" srcset="{{asset('images/contentBox/top_image03_sp.png 2x')}}">
                        <source media="(min-width: 768px)" srcset="{{asset('images/contentBox/top_image03_pc@2x.png 2x')}}">
                        <img class="mvPicture" src="{{asset('images/contentBox/top_image03_pc@2x.png')}}" alt="上下肢障がいの女性従業員。">
                    </picture>
                    <div class="interviewTitle">
                        <span class="t1">人事部</span>
                        <span class="t2">上下肢障がい</span>
                    </div>
                </a>
            </div>
            <div class="interview-item">
                <a href="{{route('interview')}}/#interview-02">
                    <picture>
                        <source media="(max-width: 767px)" srcset="{{asset('images/contentBox/top_image04_sp.png 2x')}}">
                        <source media="(min-width: 768px)" srcset="{{asset('images/contentBox/top_image04_pc@2x.png 2x')}}">
                        <img class="mvPicture" src="{{asset('images/contentBox/top_image04_pc@2x.png')}}" alt="聴覚障がいの女性従業員">
                    </picture>
                    <div class="interviewTitle">
                        <span class="t1">営業推進本部</span>
                        <span class="t2">聴覚障がい</span>
                    </div>
                </a>
            </div>
        </div>
        <div class="btnLink">
            <a class="btn-link" href="{{route('interview')}}">社員インタビューを見る</a>
        </div>
    </div>
</div>
